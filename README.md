...
The fork is pinned to upstream commit [`58cf97d`](https://github.com/openai/symphony/commit/58cf97d) (`fix(elixir): configure Codex app-server model via config`). We are not tracking upstream — Symphony's value to us is the harness *pattern*, captured once at this commit and adapted from there.
...
Originally forked from [openai/symphony](https://github.com/openai/symphony) at commit [`58cf97d`](https://github.com/openai/symphony/commit/58cf97d). Symphony is © 2025 OpenAI, licensed under Apache 2.0. The original `LICENSE` and `NOTICE` files are preserved in this repository unchanged.
